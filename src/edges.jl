...
"""
function get_callee_full_names(callee, functions_in_scope, module_paths)
    ...
    elseif get_kind(callee) === "."
        ...
        return Set([mod_path * ":" * string(func)])
    else
        return Set{String}()
    end
end
...
"""
function get_edges(defs_and_calls, path, functions_in_scope, explicit_functions, implicit_functions, module_paths, input_dir)
